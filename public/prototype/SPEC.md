...
| `#projects`, `#project.new`, `#project.new.<draft>.<step>`, `#project.<id>` | Client projects: list, post (from a Blueprint, steps in history), ranked matches and responses | projects.js |
...
| `#rates`, `#rates.<roleCategory slug>` | Rate Index + rate estimator + fractional vs full-time calculator, per-role pages | insights.js |
...
| Newsletter | One list, The Fractional GTM Pulse, every Monday (Home and Insights share it) | One destination, one voice |
| Founder on the platform | Matt Lopez's profile carries a founder line and a "Revenue Nomad founder" pill on cards; he is never featured or listed in editorial "strongest here" modules | Honest disclosure; no self-promotion |
