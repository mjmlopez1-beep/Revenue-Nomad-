/* Join: sign-up decision (#join), operator intake (#join.operator), after-submit landing (#join.done).
   ...
   - Order: role category and title (L62) -> identity and location (Country -> Postal code -> derived
     city and time zone, L18/L63/L64; "Willing to work US time zone hours?" only outside the US, L54)
     -> company fit (revenue range, then GTM motion experience, L74; employee range; industries up to 10)
     -> role details (RN.fields.roleFields; methodology "Other" opens a suggestion field, L20)
     -> availability (new client capacity 1 to 10, "Maximum 10 clients", L304/L505)
     -> fit tags (up to 25, suggestions biased to the role category, L303)
     -> headline, bio ("2-3 sentences", L65), photo (its own upload, L29), intro video with "Update video" (L30)
   ...
   - Submit saves the profile to RN.store.state.pending and lands on a page that explains the review and
     when the profile shows in the directory (comment H328), with "View dashboard" (L460). */
(function () {
  'use strict';
  const RN = window.RN;
  ...
  const STEPS = [
    ...
    { k: 'you', label: 'About you', title: 'Who you are and where you work', sub: 'Clients filter by region and time zone. We work out your city and time zone from your postal code.' },
    { k: 'fit', label: 'Company fit', title: 'The companies you do your best work with', sub: 'These answers power Match Signals: every client who opens your profile sees how well you fit their company.' },
    { k: 'details', label: 'Role details', title: 'Your operating range', sub: 'The numbers clients compare first. They show in the Operating range section of your profile.' },
  ...
  const detailKeys = (cat) => (RN.fields.roleFields[cat] || []).filter((k) => k !== 'salesMotions'); // GTM motion is asked for every role in Company fit (L74)
  const catLabel = (cat) => RN.fields.catLabel(cat);
  ...
  function errorsFor(n, d) {
    ...
    const inRange = (k) => {
      ...
      if (empty(d[k])) return;
      const v = +d[k];
      if (isNaN(v) || (f.min != null && v < f.min) || (f.max != null && f.type === 'number' && v > f.max)) e[k] = f.type === 'number' && f.max != null ? `Enter a number from ${f.min || 0} to ${f.max}` : `Enter at least ${f.type === 'money' ? RN.fmt.usd(f.min) : f.min}`;
    };
    if (n === 1) {
    ...
    if (n === 3) ['revenueRange', 'salesMotions', 'employeeRange', 'industries'].forEach((k) => need(k));
    if (n === 4) detailKeys(d.roleCategory).forEach((k) => { if (!RN.fields[k].optional) need(k); if (['number', 'money'].includes(RN.fields[k].type)) inRange(k); });
    if (n === 5) {
      ...
      if (!e.newClientCapacity && !(Number.isInteger(+d.newClientCapacity) && +d.newClientCapacity >= 1 && +d.newClientCapacity <= 10)) e.newClientCapacity = 'Enter a whole number from 1 to 10';
  ...
  function stepSummary(n, d) {
    ...
    if (n === 4) return d.roleCategory ? catLabel(d.roleCategory) : '';
  ...
  function topBar(s) {
    ...
    return `<div class="jn-top">
      <span class="step-count">${isReview ? 'Last step · Review' : `Step ${s.step} of ${TOTAL}`}</span>
  ...
  function foot(s, primary) {
    const back = s.returnTo === 'review' ? '' : `<button type="button" class="btn btn-line jn-back" data-act="join-back" aria-label="Back">${icon('arrow-left')}<span>Back</span></button>`;
    return `<div class="jn-foot">${back}<span class="grow"></span>${primary}</div>`;
  }
  ...
  function rateHint(d) {
    ...
    return `<div class="jn-rate">
      <div class="jn-rate-hd"><span class="label">Rate Index · ${esc(cat)}</span><span class="tiny muted">Illustrative</span></div>
      ...
      <p class="small">Middle half <b>${RN.fmt.usd(idx.p25)} to ${RN.fmt.usd(idx.p75)}/hr</b>, median <b>${RN.fmt.usd(idx.p50)}/hr</b>.${rate ? ` Your ${esc(RN.fmt.rate(rate))} sits ${where}.` : ''}${monthly ? ` At ${esc(RN.w.label('hoursPerMonth', d.hoursPerMonth))} a typical engagement is about ${RN.fmt.usd(monthly)}/mo.` : ''}</p>
    </div>`;
  }
  function demandStrip(d) {
    ...
    return `<div class="jn-demand">
      <div class="jn-demand-hd"><span class="label">${icon('trend-up')}In demand in ${esc(catLabel(d.roleCategory))}</span><span class="tiny muted">Client searches, last 30 days · illustrative</span></div>
  ...
  function mediaStep(d) {
    ...
    const photo = `<div class="jn-mcard">
      ...
      <div class="jn-photo">
        ...
        <div class="stack" style="--gap:10px">
          <p class="small muted">A clear, recent headshot. Cards with a photo are opened 2.3x as often.</p>
    ...
    const video = `<div class="jn-mcard">
      ...
      ${frame}
      <p class="tiny muted">Shown as “Meet ${esc(first)}” on your profile. Profiles with video hold attention 40% longer.</p>
    </div>`;
    return `<div class="jn-media">${photo}${video}</div>`;
  }

  function stepBody(n, d) {
    const f = (k, o) => RN.w.field(k, d[k] == null ? (['multi', 'tags', 'tagsearch'].includes(RN.fields[k].type) ? [] : '') : d[k], o);
    if (n === 1) return `${f('roleCategory', { help: '' })}
      <div data-jn-role ${d.roleCategory ? '' : 'hidden'}>${d.roleCategory ? roleSelect(d) : ''}</div>`;
    if (n === 2) {
      const loc = locate(d);
      return `<div class="jn-2">${f('fullName')}${f('email', { help: 'We send your review status here.' })}</div>
        ${f('linkedin', { help: 'The team uses it to check your work history.' })}
        <div class="jn-group">
          ...
          <div data-jn-ush ${d.country && d.country !== US ? '' : 'hidden'}>${f('usHours', { help: 'Shown on your profile as US time zone support, and used as a filter.' })}</div>
        </div>
    ...
    if (n === 3) return `${f('revenueRange')}
      ${f('salesMotions', { help: 'Pick every motion you have run. Clients filter by GTM motion.' })}
      ${f('employeeRange')}
      ${f('industries')}`;
    if (n === 4) {
      if (!d.roleCategory) return RN.ui.empty({ icon: 'user', title: 'Pick your role first', body: 'Role details depend on the discipline you lead.', cta: '<button type="button" class="btn btn-sm" data-act="join-goto" data-step="1">Go to Role</button>' });
      ...
      return `<div class="jn-cat-tag">${RN.ui.catDot(d.roleCategory)}<span>${esc(catLabel(d.roleCategory))}${d.role ? ' · ' + esc(d.role) : ''}</span></div>
        ${detailKeys(d.roleCategory).map((k) => k === 'methodologies'
    ...
    if (n === 5) return `${f('availability')}
      <div class="jn-2">${f('startDate', { help: 'Filled in from your availability. Change it if you know the date.' })}${f('newClientCapacity', { help: 'Maximum 10 clients.' })}</div>
      ${f('hoursPerMonth')}
      <div class="jn-eng">${f('engagementTypes', { help: '' })}
        <dl class="jn-legend">${RN.fields.engagementTypes.options.map((o) => `<div><dt>${esc(o.l)}</dt><dd>${esc(o.d || '')}</dd></div>`).join('')}</dl></div>
    ...
    if (n === 6) {
      ...
      return `<div class="note info jn-tagnote">${icon('seal')}<div><b>How fit tags work.</b> Add up to 25 self-claimed tags. Clients see them as focus areas. A tag turns <b>Verified</b> when a client review rated 4.0 or higher confirms it, and verified tags rank first.</div></div>
        ${demandStrip(d)}
  ...
  function reviewSections(d) {
    const row = (key, label) => ({ label: label || RN.fields[key].label, v: val(key, d[key]), key });
    const loc = locate(d);
    const secs = [
      { n: 1, rows: [row('roleCategory'), row('role')] },
      { n: 2, rows: [row('fullName'), row('email'), { label: RN.fields.linkedin.label, v: d.linkedin || '', wide: true }, { label: 'Location', v: place(d) }, { label: 'Time zone', v: loc.tz }].concat(d.country && d.country !== US ? [row('usHours', 'US time zone support')] : []) },
      { n: 3, rows: [row('revenueRange'), row('salesMotions'), row('employeeRange'), Object.assign(row('industries'), { wide: true })] },
      { n: 4, rows: d.roleCategory ? detailKeys(d.roleCategory).map((k) => Object.assign(row(k), { optional: RN.fields[k].optional })).concat((d.methodologies || []).includes('Other') && d.methodologyOther ? [{ label: 'Suggested methodology', v: d.methodologyOther, optional: true }] : []) : [{ label: 'Role details', v: '' }] },
      ...
      { n: 6, rows: [{ label: `${RN.fields.fitTags.label} (${(d.fitTags || []).length} of 25)`, html: (d.fitTags || []).length ? RN.ui.ftags(d.fitTags.map((t) => ({ t, tier: 'claimed' }))) : '', wide: true }] },
    ...
    return secs.map((sec) => {
      ...
      return `<section class="jn-rev ${bad ? 'is-bad' : ''}" aria-labelledby="jn-rev-${sec.n}">
        <div class="jn-rev-hd">
          <div><span class="label">Step ${sec.n}</span><h2 class="h4" id="jn-rev-${sec.n}">${esc(STEPS[sec.n].label)}${sec.n === 4 && d.roleCategory ? ` <span class="muted jn-rev-cat">· ${esc(catLabel(d.roleCategory))}</span>` : ''}</h2></div>
          <button type="button" class="btn btn-line btn-sm" data-act="join-edit" data-step="${sec.n}" aria-label="Edit ${esc(STEPS[sec.n].label)}">${icon('edit')}Edit</button>
        </div>
  ...
  function renderReview(s) {
    ...
    return `${topBar(s)}
      <header class="jn-head"><h1 class="h2">${esc(STEPS[REVIEW].title)}</h1><p class="lede">${esc(STEPS[REVIEW].sub)}</p></header>
      <div class="jn-rev-grid">
        ...
        <aside class="jn-rev-side">
          ...
          <div class="card-flat jn-after">
            ...
            <ol>
              <li>The team reviews your profile within 2 business days.</li>
              <li>It goes live in the directory at Reputation Index 50, tier Vetted.</li>
              <li>Studio shows who viewed you from day one.</li>
            </ol>
          </div>
        </aside>
      </div>
  ...
  function renderFlow() {
    const s = draft();
    if (s.step < 1 || s.step > REVIEW) s.step = 1;
    const d = s.data;
    const st = STEPS[s.step];
    let main;
    if (s.step === REVIEW) main = renderReview(s);
    else {
      const title = s.step === 4 && d.roleCategory ? `Your ${catLabel(d.roleCategory)} operating range` : st.title;
      ...
      main = `${topBar(s)}
        <header class="jn-head"><span class="eyebrow">${esc(st.label)}</span><h1 class="h2">${esc(title)}</h1><p class="lede">${esc(st.sub)}</p></header>
  ...
  function goStep(n, opts) {
    ...
    RN.store.save();
    if (RN.path() !== 'join.operator') RN.go('join.operator'); else RN.render();
  }
  ...
  RN.actions['join-tag-demand'] = (el) => {
    ...
    if (tags.length >= 25) { RN.ui.toast('Pick up to 25. Remove one to add another.', { icon: 'info' }); return; }
  ...
  const SAMPLE_DETAILS = {
    sales_leadership: { largestTeamManaged: '10-25', largestTeamQuota: '10_25m', salesCycle: ['30 - 90 days', '3 - 6 months'], methodologies: ['MEDDPICC', 'Challenger'] },
    marketing: { largestBudget: '2500000', channelsRun: ['Paid Search', 'ABM', 'Content'], b2bShare: '85' },
    revenue_operations: { crm: 'HubSpot', stackComplexity: 'complex', builtFromZero: 'yes' },
    sales_enablement: { largestRepCount: '120', enablementFocus: ['Program builder', 'Onboarding specialist'], audienceSpecialty: ['AE', 'SDR / BDR'], methodologies: ['MEDDPICC'], builtFromZero: 'yes' },
    customer_success_growth: { bestNrr: '118', largestArrBook: '15000000', csMotion: ['High-touch', 'Tech-touch / digital CS'] },
    ai_gtm: { aiSpecialization: 'Outbound / prospecting AI', codeCapability: 'functional', automationScale: 'production' },
    partnerships: { partnershipMotion: 'Tech / ISV partnerships', partnerRevenue: '12000000', builtFromZero: 'yes' },
    sellers: { individualQuota: '1200000', avgDealSize: '75000', salesCycle: ['30 - 90 days', '3 - 6 months'], methodologies: ['MEDDPICC'], commissionOnly: 'no' },
  };
  const SAMPLE_COPY = {
  ...
  RN.actions['join-submit'] = () => {
    ...
    const roleDetails = {};
    detailKeys(cat).forEach((k) => {
      if (empty(d[k])) return;
      const t = RN.fields[k].type;
      roleDetails[k] = t === 'money' || t === 'number' ? +d[k] : d[k];
    });
    ...
    const profile = {
      ...
      usHours: d.country !== US ? d.usHours : undefined,
      revenueRange: d.revenueRange, salesMotions: d.salesMotions, employeeRange: d.employeeRange, industries: d.industries,
      roleDetails, methodologyOther: (d.methodologies || []).includes('Other') ? String(d.methodologyOther || '').trim() : '',
    ...
    RN.track('signup_submit', { meta: { appId: id, roleCategory: cat, role: d.role, tags: (d.fitTags || []).length, completeness: [d.photo, d.video].filter(Boolean).length } });
    RN.mail(profile.email, 'We received your profile',
      `Hi ${profile.first},\nThanks for applying to Revenue Nomad as a Fractional ${profile.role}. The team reviews every application within 2 business days, so expect an update by ${dayName(reviewBy)}.\n\nWhat happens next:\n1. We check your work history and LinkedIn.\n2. Your profile goes live in the directory at Reputation Index 50, tier Vetted.\n3. Request reviews from 3 past clients to reach Proven.\n\nStudio shows who viewed you from the day you go live.\nReference: ${id}`, 'signup');
    RN.mail('hello@revenuenomad.com', `New operator application: ${profile.name}, Fractional ${profile.role}`,
      `${catLabel(cat)} · ${profile.location || profile.country} · ${RN.fmt.rate(profile.rate)} · ${RN.w.label('availability', profile.availability)}\n${RN.fmt.plural((profile.fitTags || []).length, 'fit tag')}, ${RN.fmt.plural((profile.industries || []).length, 'industry', 'industries')}, photo ${profile.photo ? 'added' : 'missing'}, video ${profile.video ? 'added' : 'missing'}.\nReview it in Admin, Approvals. Approve profile, then Generate score to publish at 50.\nReference: ${id}`, 'admin');
    if (videoURL) { URL.revokeObjectURL(videoURL); videoURL = null; }
    RN.go('join.done');
    ...
    return `<section class="wrap jn-hero">
      ...
      <section class="wrap jn-paths" aria-label="Choose how you use Revenue Nomad">
        ...
        <article class="jn-path jn-path-op">
          ...
          <p class="jn-path-lede">Apply in about 8 minutes. Seven short steps, saved as you go.</p>
          <ul class="jn-gets">
      ...
      <section class="wrap" aria-label="Network facts"><div class="jn-facts">
        <div><b class="num">${RN.fmt.int(RN.model.ops.length)}</b><span>operators on the network</span></div>
  ...
  function renderDone() {
      ...
      return `<section class="night jn-done-hero"><div class="wrap">
        ...
        <section class="wrap jn-done-body"><div class="jn-done-grid">
          ...
          <aside class="jn-done-side"><div class="card jn-dash">
            ...
            <ol class="jn-steplist">${STEPS.slice(1, REVIEW).map((st) => `<li>${esc(st.label)}</li>`).join('')}</ol>
            <p class="small muted">Seven short steps, one question group each. Your draft saves as you go.</p>
    ...
    const status = { in_review: ['In review', 'pill-warn'], approved: ['Live', 'pill-good'], live: ['Live', 'pill-good'], changes_requested: ['Changes requested', 'pill-bad'] }[app.status] || ['In review', 'pill-warn'];
    const live = status[0] === 'Live';
    ...
    return `<section class="night jn-done-hero"><div class="wrap">
        ...
        <p class="lede">${live ? 'Your profile shows in Browse and search. Studio now tracks who views it.' : `The team reviews every application within 2 business days. Expect an email at <b>${esc(p.email)}</b> by ${esc(dayName(reviewBy))}.`}</p>
        <div class="jn-done-meta">
          <span class="pill ${status[1]}">${esc(status[0])}</span>
        ...
        <div class="jn-done-grid">
          ...
          <aside class="jn-done-side">
            <div class="jn-side-block">
              ...
              ${previewCard(d)}
              <p class="small muted jn-visible">${icon('eye')}<span>${live ? 'Visible in Browse and search now.' : `Visible in Browse and search after approval, by ${esc(dayName(reviewBy))}.`}</span></p>
            </div>
            <div class="card jn-dash">
              <span class="jn-dash-ico">${icon('chart')}</span>
              <h3 class="h4">Your dashboard is ready</h3>
              <p class="small muted">Studio shows who viewed you, the searches you appeared in and how your rate compares with the Rate Index. It fills in once your profile is live.</p>
              <button type="button" class="btn btn-block" data-act="join-dashboard">View dashboard${icon('arrow')}</button>
              <p class="tiny muted">Prototype note: Studio opens as sample operator Matt Lopez, with his data.</p>
            </div>
          </aside>
        </div>
        <div class="jn-prep">
          ...
          <div class="jn-prep-grid">
            ...
            <article class="jn-prep-card">${icon(missing.length ? 'video' : 'check-circle')}<h3 class="h5">${missing.length ? `Add your ${esc(missing.join(' and '))}` : 'Photo and video added'}</h3><p class="small muted">${missing.length ? 'Profiles with a photo and a short video get opened and watched more. Add them from Studio any time.' : 'Your profile starts with the two things clients notice first.'}</p><a class="act" href="#guides">Read the operator guides${icon('arrow')}</a></article>
  ...
  function timeline(app, gain, reached) {
    ...
    const reviewBy = addBiz(sub, 2);
    const live = app && (app.status === 'approved' || app.status === 'live');
    const items = [
      ...
      { st: live ? 'done' : app ? 'now' : '', ic: 'shield', when: app ? `By ${dayName(reviewBy)}` : 'Within 2 business days', t: 'Team review', d: 'We check your work history and LinkedIn, and may ask for a short call. If something needs a change, we email you with what to fix.' },
      ...
      { st: '', ic: 'chart', when: 'From day one', t: 'Studio shows who viewed you', d: 'See company size and industry of every visitor, the searches you appeared in, and when you were compared and not chosen.' },
    ];
    return `<span class="eyebrow">What happens next</span>
      <h2 class="h3 jn-tl-title">From review to your first client review</h2>
      <ol class="jn-tl">${items.map((x) => `<li class="${x.st}"><span class="jn-tl-dot">${icon(x.st === 'done' ? 'check' : x.ic)}</span><div><span class="jn-tl-when">${esc(x.when)}</span><h3 class="h4">${esc(x.t)}</h3><p>${esc(x.d)}</p></div></li>`).join('')}</ol>`;
  }
  RN.actions['join-dashboard'] = () => {
    RN.store.set('persona', 'operator');
    RN.shell.renderHeader();
    RN.shell.renderDock();
    RN.go('studio');
    RN.ui.toast('Studio opened. In the prototype it shows sample operator Matt Lopez’s data.', { icon: 'info', ms: 4600 });
  };
